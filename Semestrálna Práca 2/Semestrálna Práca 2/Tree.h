...
#include<stdexcept>
	...
	{
		TreeNode<T>* result = this->shallowCopy();
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (this->isRoot()) {
			...
			throw std::logic_error("Koren nema brata");
		}
		else {
			return this->parent_->getSon(brothersOrder);
		}
	}
	//vracia  velkost podstromu
	template<typename T>
	...
	inline Tree<T>& Tree<T>::operator=(const Tree<T>& other)
	{
		if (this != &other)
		{
			...
			if (other.root_ != nullptr) {
				this->root_ = other.root_->deepCopy();
			}
		}
		return *this;
	}
