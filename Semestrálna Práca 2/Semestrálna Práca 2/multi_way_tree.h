...
#include "array_list.h"
	...
	{
		if (this->children_->size() <= order || order <0) {
			throw std::exception("Zly index syna");
		}
		else {
	...
	inline void MultiWayTreeNode<T>::insertSon(TreeNode<T>* son, int order)
	{
		if (this->children_->size() < order || order < 0) {
			...
			this->children_->insert(son, order);
		}
	}
	...
	inline TreeNode<T>* MultiWayTreeNode<T>::replaceSon(TreeNode<T>* son, int order)
	{
		if (this->children_->size() <= order || order < 0) {
			...
			this->children_->removeAt(order);
			this->children_->insert(order);
		}
	}
