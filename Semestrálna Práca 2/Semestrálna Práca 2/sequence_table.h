...
#include <stdexcept>
	...
	inline T& SequenceTable<K, T>::operator[](const K key)
	{
		...
		if (item == nullptr) {
			throw std::logic_error("Neexistuje taky kluc");
		}
		else {
	...
	inline const T SequenceTable<K, T>::operator[](const K key) const
	{
		...
		inline void SequenceTable<K, T>::insert(const K & key, const T & data)
		{
			/*if (this->containsKey(key)) {
				throw std::logic_error("Tabulka uz obsahuje kluc");
			}*/
			//else {
		...
		{
			TableItem<K, T>* itema = this->findTableItem(key);
			
			if (itema == nullptr) {
				throw std::logic_error("Tabulka neobsahuje dany kluc");
			}
			T data = itema->accessData();
			this->list_->tryRemove(itema);
			delete itema;
			return data;
		}
