...
{
	Array<T>* novePole = new Array<T>(2 * this->array_->size());
	Array<T>::copy(*this->array_, 0, *novePole, 0, this->size_);
	delete this->array_;
	this->array_ = novePole;
}
...
{
	const ArrayListIterator& otherALI = dynamic_cast<const ArrayListIterator&>(other);
	return this->position_ != otherALI.position_ || this->arrayList_ != otherALI.arrayList_;
}
